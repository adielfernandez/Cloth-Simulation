...
void PointMass::updateInteractions(float mouseInfluenceSize, float mouseInfluenceScalar, float mouseTearSize){
    ...
    if( ofGetMousePressed( 2 ) ){
        ...
        if (distanceSquared < mouseTearSize * mouseTearSize)
            links.clear();
    }
...
void PointMass::solveConstraints(float stiff, float tearDist, bool allowTearing){
    /* Link Constraints */
    ...
    if (pos.y > ofGetHeight()-1)
        pos.y = 2 * (ofGetHeight() - 1) - pos.y;
    
    if (pos.x > ofGetWidth()-1)
        pos.x = 2 * (ofGetWidth() - 1) - pos.x;
    if (pos.x < 1)
        pos.x = 2 * (1) - pos.x;
